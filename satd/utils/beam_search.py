...
class BeamSearchScorer:
    ...
    def process(
        ...
        # TODO: 遍历每个假设（Batch_size个）
        for batch_idx, beam_hyp in enumerate(self._beam_hyps):
            # 如果已经完成
            if self._done[batch_idx]:
                assert len(beam_hyp) >= self.beam_size
                # pad the batch
                # TODO: 对于已经完成的假设，将其得分置为0，token置为PAD，索引置为batch_idx * beam_size
            ...
            beam_idx = 0
            # TODO: 遍历这一步的每个假设（beam_size个）
            for beam_token_rank, (next_score, next_token,
                ...
                # TODO: 计算当前假设在input_ids中的位置
                batch_beam_idx = batch_idx * self.beam_size + next_index
                # TODO: 判断当前假设是否使得其所在的预测序列完成
                l2r_done = (input_ids[batch_beam_idx][0].item()
                            == vocab.SOS_IDX
                            and next_token.item() == vocab.EOS_IDX)
                r2l_done = (input_ids[batch_beam_idx][0].item()
                            == vocab.EOS_IDX
                ...
                # TODO: 如果没有结束，将当前假设的得分、token、索引加入到下一步的预测中
                else:
                    # TODO: 加入下一个预测的token
    ...
    def finalize(
        ...
        scores: FloatTensor = torch.zeros(self.batch_size * self.beam_size,
                                          dtype=torch.float,
                                          device=self.device)
